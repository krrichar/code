...
with open('jenkins_non-prod.json') as f:
    json = json.load(f)
...
def vlan_cfg(vlan, desc, trunks):
    ...
    for trunk in trunks:
        sub_commands = [
        'interface {}'.format(trunk),
        'switchport trunk allowed vlan add {}'.format(vlan)'
        ]
        net_connect.send_config_set(sub_commands)
